...
class Api_v100_model extends CI_Model {
    ...
    function validate_user($email   =   '' , $password   =  '', $is_corporate   =  ''){
        $result = FALSE;
        $credential    =   array(  'email' => $email , 'password' => $password , 'is_corporate' => $is_corporate);
    ...
    // validate login  function
    function validate_corporate_user($email   =   '' , $password   =  ''){
        $result = FALSE;
        $credential    =   array(  'email' => $email , 'password' => $password , 'is_corporate' => '1');
        $query = $this->db->get_where('user' , $credential);
        if ($query->num_rows() > 0):
            $result = TRUE;
        endif;    
    ...
    function check_meeting_status($meeting_code = '') {
        $credential = array('meeting_code' => $meeting_code);
        $result = $this->db->get_where('meeting', $credential)->row();
        return $result;
    }
    ...
    // get user info  function
    function create_user($name='',$email   =   '' , $password   =  '', $is_corporate   =  ''){
        $data['name']           = $name;
        $data['email']          = $email;
    ...
    function get_users_schedule($user_id = '') {
       /* $this->db->where('user_id', $user_id);
        $this->db->where('is_opened', "false");
        return $this->db->get('meeting')->result_array();*/
        
        $query = $this->db->query("SELECT * FROM `meeting` WHERE Now() < DATE_ADD(scheduled_at, INTERVAL 1 HOUR) and user_id='$user_id' and is_opened='false'")->result_array();
        return $query;
    ...
    function open_scheduled_meeting($meeting_id = '') {
        $this->db->where('meeting_id',$meeting_id);
        $data = array("is_opened" => "true");
    ...
    */
    ...
    function create_meeting($data= array(),$history = false){
        ...
        if($history):
            ...
            $history_data['nick_name']   = $data['meeting_title'];
