...
class Common_model extends CI_Model {
    ...
    */
    ...
    public function get_cuser_num_rows($name='')
    {
        if($name !="" && $name !=NULL):
            $this->db->like("name",$name,"both");
        endif;
        $this->db->where('login_status','disapproved');
        $this->db->or_where('login_status','verified');
        return $this->db->get('user')->num_rows();
    }
    
     public function get_corporate_users($name='',$limit=NULL, $start=NULL)
    {
        if($name !="" && $name !=NULL):
            $this->db->like("name",$name,"both");
        endif;
        $this->db->where('admin_verifcation','corporate_disapproved');
        $this->db->or_where('admin_verifcation','corporate_verified');
        $this->db->order_by('user_id',"DESC");
        $this->db->limit($limit,$start);
        return $this->db->get('user')->result_array();
    }
    
     public function get_news_num_rows($name='')
    {
        if($name !="" && $name !=NULL):
            $this->db->like("name",$name,"both");
        endif;
     ...
     public function get_newsletter($name='',$limit=NULL, $start=NULL)
    {
        if($name !="" && $name !=NULL):
            $this->db->like("name",$name,"both");
        endif;
    ...
    */
    ...
    function create_meeting_join_history($data = array()){
       // date_default_timezone_set("Asia/Kolkata");
        ...
        $history_data['joined_at']      =  $data['joined_at'];
        $this->db->insert("meeting_history", $history_data);
    }
    ...
    function get_join_meeting_count1($data){
       $query = $this->db->query("SELECT * FROM `meeting_history` where DATE(joined_at)='$data'");
    ...
    function get_host_meeting_count1($data){
        $query = $this->db->query("SELECT * FROM `meeting` where DATE(created_at)='$data'");
        $query1 = $query->num_rows();
        return $query1;
       /* $this->db->like("created_at",$data,"both");
        return $this->db->get('meeting')->num_rows();*/
    ...
    function joined_meeting_this_month_chart_data(){
        ...
        for ($i=1; $i <$days ; $i++):
            ...
            $data .= $this->get_join_meeting_count1($yearmonth.'-'.$i);
        endfor;
        return $data;
    }
    ...
    function hosted_meeting_this_month_chart_data(){
        ...
        for ($i=1; $i <$days ; $i++):
            ...
            $data .= $this->get_host_meeting_count1($yearmonth.'-'.$i);
        endfor;
        return $data;
    }
