...
class Admin extends CI_Controller {   
   ...
   /* function frontpage($id){
     $call_from_any_device = $this->input->post('call_from_any_device');
     $call_from_any_device = $this->input->post('call_from_any_device');
     $call_from_any_device = $this->input->post('call_from_any_device');
     $call_from_any_device = $this->input->post('call_from_any_device');
    }*/
    ...
    function newsletters(){
        ...
        $total_rows     = $this->common_model->get_news_num_rows($name);
        // page
        $config                     = $this->common_model->pagination();
        $config["base_url"]         = base_url() . "admin/newsletter?".$search_string;
        ...
        $data["news"]              = $this->common_model->get_newsletter($name,$config["per_page"], $page);
    ...
    function status_change($id){
        $query = $this->db->query("select * from user where user_id='$id'")->row();
        $status = $query->admin_verifcation;
        if($status=='disapproved'){
                                         $data['admin_verifcation'] = 'verified';
                                         }
                                         else{
                                         $data['admin_verifcation'] = 'disapproved';
                                         }
    ...
    function cstatus_change($id){
        $query = $this->db->query("select * from user where user_id='$id'")->row();
        $status = $query->admin_verifcation;
       
                                         $data['admin_verifcation'] = 'corporate_verified';
                                        
                                         
            $this->db->where('user_id', $id);
            $this->db->update('user', $data);
            $this->session->set_flashdata('success', 'User update successed.');
            redirect(base_url() . 'admin/corporate_user/', 'refresh');
    }
    
     function cstatus_change1($id){
        $query = $this->db->query("select * from user where user_id='$id'")->row();
        $status = $query->admin_verifcation;
                                         
                                         $data['admin_verifcation'] = 'corporate_disapproved';
...
function corporate_user($param1 = '', $param2 = ''){
        // active menu session
        $this->session->unset_userdata('active_menu');
        ...
        $total_rows     = $this->common_model->get_cuser_num_rows($name);
        // page
        $config                     = $this->common_model->pagination();
        $config["base_url"]         = base_url() . "admin/corporate_user?".$search_string;
        $config["total_rows"]       = $total_rows;
        $config["per_page"]         = 15;
        $config["uri_segment"]      = 3;          
        //$config['use_page_numbers'] = TRUE;
        ...
        $data['last_row_num']       =  $this->uri->segment(3);
        $page                       = ($this->input->get('per_page') !="" || $this->input->get('per_page') !=NULL) ? $this->input->get('per_page') : 0;//($this->uri->segment(3)) ? $this->uri->segment(3) : 0;   
        $data["users"]              = $this->common_model->get_corporate_users($name,$config["per_page"], $page);
        $data["links"]              = $this->pagination->create_links();
        $data['total_rows']         = $config["total_rows"];
        $data['page_name']          = 'corporate_user';
        $data['page_title']         = 'User Management';             
        $this->load->view('admin/index', $data);
    }
...
//}
   ...
   /*     if ($this->session->userdata('admin_is_login') != 1){
        ...
        $data['page_title']     = 'Send Notification(OneSignal)';
        $this->load->view('admin/index', $data);
    //}
    }
    ...
    function backup_restore($operation = '', $type = ''){
        ...
        $data['page_info']  = 'Create Database Backup';
        $data['page_name']  = 'backup_restore';
        $data['page_title'] = 'Create Database Backup';
        $this->load->view('admin/index', $data);
    }
    ...
    function test_mail(){
        ...
        if($email !=''){
            ...
            if($this->email_model->test_mail($email)){
               ...
               // echo 'Mail Configuration is perfect';
              // redirect(base_url() . 'admin/email_setting/', 'refresh');
            }            
        }
        else{
