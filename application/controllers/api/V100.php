...
class V100 extends RestController{ 
    ...
    public function login_post() {
        ...
        $is_corporate = trim($this->input->post('is_corporate'));
        if (filter_var($email, FILTER_VALIDATE_EMAIL) && $password !='' && $password !=NULL){            
            $login_status               = $this->api_v100_model->validate_user( $email ,$password,$is_corporate);   
          
          
            $login_stt = $this->db->query("select * from user where email='$email' and password='$password'")->row();
            if($login_stt){
                if($is_corporate == '0'){
                   if($login_stt->login_status == 'verified'){
                ...
                if($query->num_rows() > 0){                   
                    ...
                    $response['data']       = 'Your account is disabled.Please contact with system administrator to enable.';
                       ...
                       $response['status']     = 'error';
                   $response['data']       = 'Please verify your email';
                   }
                }
                else if($is_corporate == '1'){
                    if($login_stt->login_status == 'verified'){
                       if($login_stt->admin_verifcation == 'corporate_verified'){
                ...
                if($query->num_rows() > 0){                   
                    ...
                    $response['data']       = 'Your account is disabled.Please contact with system administrator to enable.';
                           ...
                           $response['status']     = 'error';
                   $response['data']       = 'Please wait for admin verification!';
                       }
                   }
                   else{
                       $response['status']     = 'error';
                   $response['data']       = 'Please verify your email';
                     ...
                     $response['status']     = 'error';
                   $response['data']       = 'Please use other method to login';   
                }
            }
            else{
                $response['status']     = 'error';
    ...
    public function signup_post() {
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL) && $password !='' && $password !=NULL && strlen($password) > 3):
            ...
            if ($signup_ability):
                $this->api_v100_model->create_user($name, $email ,$md5_password,$is_corporate);
                $uidd = $this->db->query("select * from user where email = '$email' order by user_id desc")->row();
                $uid = $uidd->user_id;
                $this->load->model('email_model');
                $this->email_model->account_opening_email($email, $password,$uid);
    ...
    public function user_details_by_user_id_get() {
        ...
        if (is_numeric($user_id) && $user_id !='' && $user_id !=NULL):            
            ...
            if ($is_valid_user_id):
                ...
                $response['user_id']    = $user_info->user_id;
                $response['name']       = $user_info->name;
                $response['is_corporate'] = $user_info->is_corporate;
                $response['email']      = $user_info->email;
                $response['image_url']  = $this->common_model->get_image_url('user',$user_info->user_id);
                $response['meeting_code'] = $user_info->meeting_code;
                $response['phone']      = $user_info->phone;
    ...
    public function password_reset_post() {        
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)):         
            ...
            if($user_exist):
                ...
                $this->load->model('email_model');
                if($this->api_v100_model->reset_password($email,$new_password)):
    ...
    function get_users_schedule_post() {
        $user_id              = $this->input->post('user_id');
        $schedule             = $this->api_v100_model->get_users_schedule($user_id);
    ...
    function join_meetting_post(){
        ...
        $meeting_id               =   $this->input->post('meeting_id');
         $joined_at               =   date('Y-m-d H:i:s',strtotime($this->input->post('joined_at')));
        $meeting_config = $this->api_v100_model->check_meeting_status($meeting_code);
        if($meeting_config->is_opened != "false") {
            if(empty($user_id) || $user_id =='' || $user_id ==NULL):
                $user_id             = 0;
            endif;
    
            $history_data['user_id']            =   $user_id;
             $history_data['joined_at']            =   $joined_at;
        ...
        else if ($meeting_config->is_opened == "false" && $meeting_config->user_id == $user_id){
            
            $this->db->where("meeting_id", $meeting_id);
            $this->db->update('meeting', array('is_opened' => "true"));
            ...
            $response['status']     = 'error';
            $response['message']    = $meeting_config.'The meeting has not started yet. Please try again later.';
